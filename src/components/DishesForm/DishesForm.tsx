...
function DishesForm({ data, handleChange, materials, error }: DishesFormProps) {
  ...
  function handleAdd(e: any) {
    ...
    let isCreate = dataMaterials?.filter((m: any)=> m.id === itemToAdd.idItem ) ? false : true


    if(isCreate){
      let itemComplete = {amount: itemToAdd.amount, id:completeItem?.id, name: completeItem?.name, unit: completeItem?.unit }
      dataMaterials?.push(itemComplete)
  
      handleChange({...data, materials: dataMaterials})
      return
    }
    let dataNoAlready = dataMaterials?.filter((m: any)=> m.id !== itemToAdd.idItem )
    let itemComplete = {amount: itemToAdd.amount, id:completeItem?.id, name: completeItem?.name, unit: completeItem?.unit }
  ...
  function removeMaterial(id:string) {
    ...
    let dataNoAlready = dataMaterials?.filter((m: any)=> m.id !== id )
    handleChange({...data, materials: dataNoAlready})
  }
    ...
    <>
      ...
      <div className="flex flex-wrap space-x-3 p-2">
        {data?.materials?.map((m: any) => (
          <MaterialTag name={m.name}   amount={m.amount} unit={m.unit} id={m.id} removeMaterial={removeMaterial} />
        ))}
      </div>
    </>
  );
